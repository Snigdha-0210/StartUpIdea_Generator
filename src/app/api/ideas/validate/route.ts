...
export async function POST(request: Request) {
  try {
    ...
    const { idea, industry, targetScope, specificLocation, budget, time } = body;
    if (!idea) {
      return NextResponse.json({ error: 'Idea is required' }, { status: 400 });
    }
    
    const data = await aiValidationService.validateIdea(idea, industry, targetScope, specificLocation, budget, time);
